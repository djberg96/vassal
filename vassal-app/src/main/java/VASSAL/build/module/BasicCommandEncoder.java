...
import VASSAL.counters.Comment;
...
public class BasicCommandEncoder implements CommandEncoder, Buildable {
  ...
  private final Map<String, DecoratorFactory> decoratorFactories = Map.ofEntries(
    ...
    Map.entry(TableInfo.ID, decoratorFactory(TableInfo::new)),
    ...
    Map.entry(PlaceMarker.ID, decoratorFactory(PlaceMarker::new)),
    Map.entry(Replace.ID, decoratorFactory(Replace::new)),
    Map.entry(ReportState.ID, decoratorFactory(ReportState::new)),
    ...
    Map.entry(Translate.ID, decoratorFactory(Translate::new)),
    Map.entry(AreaOfEffect.ID, decoratorFactory(AreaOfEffect::new)),
    Map.entry(CounterGlobalKeyCommand.ID, decoratorFactory(CounterGlobalKeyCommand::new)),
    ...
    Map.entry(ActionButton.ID, decoratorFactory(ActionButton::new)),
    Map.entry(GlobalHotKey.ID, decoratorFactory(GlobalHotKey::new)),
    Map.entry(Deselect.ID, decoratorFactory(Deselect::new)),
    Map.entry(Mat.ID, decoratorFactory(Mat::new)),
   ...
   *              successive Trait in the trait list presented in a piece's {@link VASSAL.counters.PieceDefiner} dialog represents a step "outward").
   */
  public Decorator createDecorator(String type, GamePiece inner) {
