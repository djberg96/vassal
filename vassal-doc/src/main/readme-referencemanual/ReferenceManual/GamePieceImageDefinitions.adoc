...
Images defined in this component will appear in the drop-down menu for selecting images for any Trait of any Game Piece just like an imported SVG, GIF, JPEG, or PNG.
Generated images are stored as SVG files in the module archive by default.
...
A Font Style defines a Font Family that can be reused by text items in Game Piece Layouts.
Size, bold, italic, outline, outline thickness, and outline color are set on the individual Label item in the layout.
A default style is always defined.
