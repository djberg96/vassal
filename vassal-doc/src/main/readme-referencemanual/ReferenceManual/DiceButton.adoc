...
==== Properties
A Dice Button exports its results to 4 Global Properties that are accessible from other Vassal components. These properties will keep their values until the next roll, including in saved games and logs.

Each property name is of the form <DiceButtonName>_<property>. The four properties exported are:
...
|===
|
*Name:*:: The Description field sets the name of Dice Button. This is used for reporting and to set global <<Properties.adoc#top,Properties>> holding the results of the roll.
...
*Report format:*:: Specifies the <<MessageFormat.adoc#top,Message Format>> for reporting the results. The report can contain any of the following properties:
+
...
_$count#$_::: The count of the number of rolls for each die side after 'keeping' or sorting. E.g., $count1$ will be the number of '1's rolled.
_$summary$_::: A summary of the roll counts in the format <value> x<count>[, <value> x<count>...] When a roll generates no results, the summary is an empty string. Since the summary text is localized to the current culture, any parsing of this text is discouraged. +

*Prompt for values when button pushed:*:: When unselected, the values for the following 4 fields are fixed for all dice rolls. When selected, the user is prompted for these values in a popup window when the Dice Button is activated. Each of the four values can be locked to a specified value rather than appearing in the prompt dialog. If all 4 fields are locked, a normal dice roll with no prompting is performed.

*Number of dice:*:: How many dice to roll.

*Lock number of dice:*:: Don't prompt for number of Dice, use the specified value.

*Number of sides per die:*:: The number of sides or faces of each die representing the maximum number of possible outcomes per die.

*Lock number of sides per die:*:: Don't prompt for number of sides, use the specified value.

*Add to each die:*:: A number to add to each roll.

*Lock add to each die:*:: Don't prompt for a number to add to each die, use the specified value.

*Add to overall total:*:: A number to add once to the overall total result.

*Lock add to overall total:*:: Don't prompt for a number to add to overall total, use the specified value.
...
An Internet Dice Button has the same configuration fields and reporting behavior as a Dice Button, but obtains roll results from the internet dice service selected in the player's Preferences.
Supported services are *random.org* and *qrandom.io*.

The internet dice service and API key are global player preferences rather than module settings.
RANDOM.ORG requires an API key.
Players can test the selected service and key from *Preferences > Internet Dice* by pressing *Verify*.

If the selected service cannot be used, the Internet Dice Button rolls locally.
Internet Dice Button report formats can use the same variables as Dice Button report formats, plus _$rollDetails$_ for the internet roll description.
