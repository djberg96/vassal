...
Font Styles used in Image Layouts are defined here and selected by name from drop-down menus.
A Font Style defines the font family used by text items.
Size, bold, italic, outline, outline thickness, and outline color are set on each Label item in the Game Piece Layout, so the same font family can be reused with different per-layout typography.
A default style is always defined: Dialog font.
...
[loweralpha, start=15]
. *Size:* The size of the label text in points.

[loweralpha, start=15]
. *Bold:* Click on to select a Bold font style for this label.

[loweralpha, start=15]
. *Italic:* Click on to select an Italic font style for this label.

[loweralpha, start=15]
. *Outline:* Click on to draw an outline around this label's text.

[loweralpha, start=15]
. *Outline Thickness:* The thickness, in pixels, of the outline drawn around this label's text.

[loweralpha, start=15]
. *Outline Color:* The color of the outline drawn around this label's text. When text is specified in individual images, each image can override this color.
...
* *Name:* Specify a name for the image definition. This is the name under which this image will appear in the image-selector drop-down menu in a Game Piece Trait's Properties. Generated images are saved as SVG files by default.
* *Bucket:* Optional image-selector bucket in which to list this generated image. Buckets help organize large image lists and appear in image chooser controls.
...
Generated images are stored as SVG in the module archive, so they scale cleanly and can be inspected or edited with external SVG tools if needed.
