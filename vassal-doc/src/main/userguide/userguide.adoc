...
*Development Team:* Rodney Kinney, Joel Uckelman, Brent Easton, Michael Kiefte, Tim McCarron, Brian Reynolds
...
=== Playing by Email (PBEM) or by Discord
...
(Vassal log files have the suffix .vlog.). Or, if you are playing via Discord, drag the log file into your chat w/ your opponent to upload it.
B. Reviewing an opponent’s email log:
. When you receive your opponent’s email (or a new log via Discord or other means), save the attached log file to your system. *TIP*: If you _already_ have a window open with an earlier turn of the same game, then if you have VASSAL 3.6.4 or higher you can simply drag the file or link directly to your existing window (drag on top of the map or chat) -- this will automatically load the game straight into your existing window and you can skip straight to step 6.
...
For more information, see <<Playing by Email (PBEM) or by Discord>>.
...
Some modules can also use *Help > Ask Rules Assistant...*.
When enabled in Preferences, the Rules Assistant reads rules and chart material included in the module, selects relevant excerpts, and sends your question plus those excerpts to the selected AI provider.
Answers are intended as play aids, not as official errata or tournament rulings; when a ruling matters, verify it against the cited rules, charts, or the publisher's current rules.

The Rules Assistant window keeps the current session's questions and answers while the module is open.
You can move backward and forward through that history, copy an answer, or save an answer or selected excerpt to Public Notes when the module includes a Notes window.
...
Internet Dice preferences control rolls made by module buttons configured as Internet Dice Buttons.
The *Internet dice service* setting selects the provider used for those rolls.
Supported services include *random.org* and *qrandom.io*.

The *API key / password* field stores the credential required by the selected service.
RANDOM.ORG requires an API key; qrandom.io does not normally require a key.
Use *Verify* to make a test d6 roll with the selected service and credential.
Use *Show* or *Hide* to reveal or mask the stored key while editing it.

If an Internet Dice Button cannot use the configured internet service, VASSAL rolls the dice locally instead.
...
Rules Assistant preferences control the optional AI rules helper available from *Help > Ask Rules Assistant...*.

* *Enable Rules Assistant:* Turns the feature on for modules opened on this computer.
* *AI provider:* Selects the provider to use. Supported providers are *OpenAI*, *xAI*, and *Manus*.
* *Model:* Selects a model. Leave this blank to use the default model for the selected provider.
* *API key:* Stores the provider API key. Use *Show* or *Hide* to reveal or mask the key while editing it.
* *Refresh Models:* For OpenAI, retrieves the available model list using the API key. For providers with fixed choices, refresh restores the known model choices.

The assistant sends your question and selected module help excerpts to the configured provider.
Do not enter private information, unpublished rules material, or other text you do not want sent to that provider.
...
* Discord has become very popular for playing Vassal either live or "by email" -- in the latter case the chat room is often a more convenient place to exchange log files than traditional email. Vassal's "official" Discord server can be found at https://discord.gg/GzzHccRQpU, and there are gamer-run discords specializing in PBEM and specific games.
